...
import {TextBlue, TextWhite, TextRed, Text} from '../../components/Text';
...
const ChangePassword: () => React$Node = () => {
  ...
  const [passwordMessageFromApi, setPasswordMessageFromApi] = useState('');
  ...
  const goToPinPage = () => {
    schema
      ...
      .then(() => {
        // clear the errors
        setValidateError({});
        dispatch(changePasswordRequest(originPassword, confirmPassword, false));
      })
      .catch(err => {
        const errors = {};
        ...
        <ScrollView
          ...
          <View
            ...
            <View
              ...
              {language === 'th' ? (
                <Text>
                  กรุณาตั้ง <TextBlue>รหัสผ่านใหม่</TextBlue>
                  {'\n'}
                  สำหรับการใช้งานครั้งแรก
                </Text>
              ) : (
                <Text>
                  Please <TextBlue>set new password</TextBlue> {'\n'}for first
                  log in.
                </Text>
              )}
            </View>
            <ContentSimple style={{flex: 1, marginTop: 15}}>
              ...
              <View style={{paddingBottom: 10}}>
                ...
                {passwordMessageFromApi !== '' ? (
                  <TextRed>{passwordMessageFromApi}</TextRed>
                ) : null}
              </View>
              <View style={{flexDirection: 'row'}}>
                <BorderRequired
